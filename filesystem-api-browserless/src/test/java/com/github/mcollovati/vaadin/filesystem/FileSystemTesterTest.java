...
class FileSystemTesterTest {
    ...
    public static class OpfsView extends Div {
        ...
        final NativeButton writeBtn = new NativeButton("Write", e -> fs.getOriginPrivateDirectory()
                .thenCompose(root -> root.getFileHandle(fileName.getValue(), GetHandleOptions.creating()))
                .thenCompose(handle -> handle.writeString(editor.getValue())));
        final NativeButton readBtn = new NativeButton("Read", e -> fs.getOriginPrivateDirectory()
                .thenCompose(root -> root.getFileHandle(fileName.getValue()))
    ...
    public static class OpfsManageView extends Div {
        ...
        final NativeButton removeRecursiveBtn = new NativeButton("Remove Recursive", e -> fs.getOriginPrivateDirectory()
                .thenCompose(root -> root.removeEntry(targetName.getValue(), RemoveEntryOptions.recursively()))
                .thenAccept(v -> result.setText("removed")));
    ...
    public static class WritableView extends Div {
        final FileSystemAPI fs = new FileSystemAPI(this);
        final Span status = new Span();
        final NativeButton seekWriteBtn = new NativeButton("Seek Write", e -> fs.getOriginPrivateDirectory()
        ...
        final NativeButton truncateBtn = new NativeButton("Truncate", e -> fs.getOriginPrivateDirectory()
                .thenCompose(root -> root.getFileHandle("test.txt", GetHandleOptions.creating()))
                .thenCompose(FileSystemFileHandle::createWritable)
                .thenCompose(w ->
    ...
    public static class KeepExistingView extends Div {
        final FileSystemAPI fs = new FileSystemAPI(this);
        final Span status = new Span();
        final NativeButton patchBtn = new NativeButton("Patch", e -> fs.getOriginPrivateDirectory()
                .thenCompose(root -> root.getFileHandle("existing.txt"))
    ...
    public static class PermissionsView extends Div {
        ...
        final NativeButton checkBtn = new NativeButton("Check", e -> fs.getOriginPrivateDirectory()
                .thenCompose(root -> root.getFileHandle("a.txt"))
    ...
    public static class SameEntryView extends Div {
        ...
        final NativeButton checkBtn = new NativeButton("Check", e -> fs.getOriginPrivateDirectory()
                .thenCompose(root -> root.getFileHandle("a.txt")
                        .thenCompose(h1 -> root.getFileHandle("a.txt").thenCompose(h2 -> h1.isSameEntry(h2))))
    ...
    public static class ResolveView extends Div {
        ...
        final NativeButton resolveBtn = new NativeButton("Resolve", e -> fs.getOriginPrivateDirectory()
